...
function ld_fvm_linear!(du, u, p, t)
...
function ld_fvm_upwind!(du, u, p, t)
    # find the value at the cells
    u_half = zeros(p.Nx - 1)
    for i = 1:(p.Nx-1)
        if u[i] > 0
...
function simul(tl, p::ParamsLA; method = "fdm")
    ...
    elseif method == "fvm_linear"
        prob = ODEProblem(ld_fvm_linear!, p.u0, (tl[1], tl[end]), p, saveat = tl)
    elseif method == "fvm_upwind"
        prob = ODEProblem(ld_fvm_upwind!, p.u0, (tl[1], tl[end]), p, saveat = tl)
    end
